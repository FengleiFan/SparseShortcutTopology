...
args.layer_nums = [(40-4)//3 for _ in range(3)]
...
def dataLoader(is_train=True, cuda=True, batch_size=64, shuffle=True):
        if is_train:
            ...
            train_set = td.CIFAR100('data', train=True, transform=trans,download=True)
            size = 50000
            ...
            trans = [transforms.ToTensor(),
                     transforms.Normalize(mean=[n/255.
                        for n in [129.3, 124.1, 112.4]], std=[n/255. for n in [68.2,  65.4,  70.4]])]
            trans = transforms.Compose(trans)
            test_set = td.CIFAR100('data', train=False, transform=trans,download=True)
            size = 10000
            train_loader = torch.utils.data.DataLoader(
...
def train():
    ...
    return total_loss.item()/training_size, corrects, corrects/training_size * 100.0
